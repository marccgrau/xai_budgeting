...
def save_results_to_csv(
    ...
    log_and_save_evaluation_results(
        Path(f"evaluations/evaluation_lasso_{category}_{acc_id}_{region}.txt"), **kwargs
    )
...
def main(
    ...
    # Filter by acc_id and region if provided
    ...
    preprocessor = joblib.load(Path(f"models/preprocessor_lasso_{category}.joblib"))

    # Split the data into test set based on year
    ...
    # Drop rows with NaN values, consistent with the training script
    ...
    model = joblib.load(Path(f"models/best_model_lasso_{category}.joblib"))
    ...
    save_results_to_csv(
        Path(f"evaluations/lasso_predictions_{acc_id}_{region}.csv"),
        y_test=y_test,
        y_pred=y_pred,
        ...
        **{
            ...
            "Model saved successfully at": str(
                Path(f"models/best_model_lasso_{category}.joblib")
            ),
        },
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluate the Lasso regression model."
    )
